...
\begin{itemize}
	\item structure and purpose of the particular dataset (textual for IE tasks, relational for classification ML tasks and RDF datasets for Semantic Web reasoning tasks), 
...
\end{itemize}
The description of individual datasets will be presented at the very end of the chapter (Section~\ref{sec:data_individual}), before that, relevant common aspects of the datasets will be discussed.
...
Classification datasets are very familiar in the community of machine learning. They are used for evaluation of propositional ML engines (like Decision Trees, Naive Bayes, Multilayer Perceptron, Support Vector Machines, etc.) on the classification task. The classification task (or problem) can be, in a simplified way, described as follows: 

Given a set of objects (e.g. accidents); each object is described using the same set of attributes (e.g. number of fatalities, number of injuries, number of intervening units; etc.) and each object is classified into one of a fixed set of classes (not serious accident, middle serious accident, very serious accident). Based on the known classification of the objects form the training set, predict the classification for new objects with unknown classification (testing set). 
...
In this section, the datasets will be divided into third party datasets and contributed ones. 

\subsection{Contributed Datasets} \label{sec:data_contributed}
...
\section{Individual Datasets} \label{sec:data_individual}
In this section individual datasets used in this thesis will be presented. The order of dataset presentations is the same as the order in which they were used in this work.
...
\hline
\end{tabular}
\caption{\nameref{sec:data_fireman_without} dataset -- statistics} \label{tab:data_fire_without}
\end{table}



In the early beginning of our work, first IE experiments were done with textual reports from fire departments of several regions of the Czech Republic. These departments are responsible for rescue and recovery after fire, traffic and other accidents. But the reports do not deal only with fire and traffic accidents but also chemical interventions, fire-fighting contests, fire drills and similar events can be found. The reports are rich in information, e.g. where and when an accident occurred, which units helped, how much time it took them to show up on the place of accident, how many people were injured, killed etc. An example of such report was presented in Figure~\ref{fig:manual_article}.
...
\begin{center}
{\footnotesize\verb+http://aplikace.mvcr.cz/archiv2008/rs_atlantic/hasici/REGION/ID.html+}
\end{center}

For example report 47443 (ID) from Jihomoravský region has following URL:

\begin{center}
{\footnotesize\verb+http://aplikace.mvcr.cz/archiv2008/rs_atlantic/hasici/jihomoravsky/47443.html+}
\end{center}


The dataset contains reports from eight Czech regions: Jihomoravský (127 reports), Královéhradecký (109 reports), Moravskoslezský (138), Olomoucký (77), Pardubický (95), Plzeňský (97), Ústecký (108) and Vysočina (63). All the reports are written in the Czech language. 

The dataset does not contain any manual annotations; only linguistic annotations produced by PDT tools are included in the dataset (FS format for Netgraph processing and PML format for Btred processing, see Section~\ref{sec:third_pdt_tools_and_resources} for details about these tools). Table~\ref{tab:data_fire_without} summarizes some properties (described bellow) of the dataset’s reports.  Total sum, average value, standard deviation, minimum, maximum and median of the values can be found in the table.



\begin{description}
	\item[Text size] is the length of the text of a particular report in characters. More precisely it is the size of the corresponding ``.txt'' file. ISO-8859-2 (Latin-2) character encoding is used in these files so the number of characters is the same as the file size in bytes. 
	\item[Number of words] in a particular report is counted using UNIX command ``wc -w''.
	\item[Annotations size] expresses the number of bytes used by the corresponding linguistic annotations of a particular report. Only annotations in FS format are counted (PML files are not included in these numbers).
	\item[Number of trees] is the number of tectogrammatical trees in a particular report. The number also corresponds with the number of sentences in the report.
\end{description}
...
\begin{description}
	...
	\item[`end\_subtree'] was obtained by the subtree postprocessing of `end' annotations. This was necessary because `end' annotations were not annotated consistently -- sometimes they include only the numeric representation of the time, sometimes they include also the unit; e.g. `16:10' versus `16:10 hours'. 'end\_subtree' always include the time unit (if available in text).
	...
	\item[`fatalities'] annotates mentions of people who died during or shortly after the accident. 
...
\hline
\end{tabular}
\caption{\nameref{sec:data_fireman_annotated} -- counts of manual annotations} \label{tab:data_fire_with}
\end{table}
...
\hline
\end{tabular}
\caption{\nameref{sec:data_corporate_acquisitions} -- counts of manual annotations} \label{tab:data_acquisitions}
\end{table}
...
The development of the Fuzzy ILP Classifier was partly motivated by the possibility of doing seriousness classification of accidents presented in the reports described in previous sections. We selected the same collection of 50 web reports as in the dataset \nameref{sec:data_fireman_annotated} and we manually extracted values of the most interesting features (These features will be described bellow.) We also assigned a value of overall ranking of seriousness of the presented accident to each report.  The value of seriousness ranking is the target attribute of the classification task. 
...
The majority of accidents are of the type \verb+fire+ (52\%)
or \verb+car_accident+ (30\%),
