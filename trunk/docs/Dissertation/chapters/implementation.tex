...
Example of a serialized tectogrammatical tree is in Figure~\ref{fig:ilp_serialization} it is the same tree as in Figure~\ref{img:ch10_damage_tree}.
...
\end{minted}
\caption{ILP serialization example based on the tree from Figure~\ref{img:ch10_damage_tree}.}
...
\end{minted}
\caption{Document ontology example (RDF serialization of linguistic trees), based on Figure~\ref{img:ch10_damage_tree}}.
...
Figure~\ref{img:rules_protege} shows the example rules in Prot\'{e}g\'{e} 4 -- Rules View's format. The last rule example can be seen in Figure~\ref{img:rules_jena}, it shows a rule in the Jena rules format\footnote{\url{http://jena.sourceforge.net/inference/#RULEsyntax}}. Conversion to Jena rules was necessary because it is the only format that Jena can parse, see details about our use of Jena in Section~\ref{sec:ch70_experiment}. The Jena rules were obtained using following transformation process:
\begin{enumerate}
