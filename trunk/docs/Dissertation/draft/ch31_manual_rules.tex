...
We propose a relatively straightforward process for the extraction of semantic data from text-based web-resources. This process consists of four steps. The Figure~\ref{img:extract_process} describes it.


We propose a relatively straightforward process for the extraction of semantic data from text-based web-resources. This process consists of four steps. The Figure~\ref{img:extract_process} describes it.

\begin{enumerate}
\item \textbf{Extraction of text} \\ The linguistic annotating tools process plain text only. In this phase we have to extract the text from the structure of a given web-resource. In this first phase we have used RSS feed of the fire department web-page. From this we have obtained URLs of particular articles and we have downloaded them. Finally we have extracted the desired text by means of a regular expression.

%Lingvistické anotátory zpracovávají prostý text, který v této fázi musíme z~webové stránky získat.

\item \textbf{Linguistic annotation} \\ In this phase the linguistic annotators process the extracted text and produce corresponding set of dependency trees representing the deep syntactic structure of individual sentences. We have used the linguistic tools described in the section~\ref{sec:ling_tools} for this task.

%Extrahovaný text pøedložíme lingvistickému anotátoru, který v textu rozpozná jednotlivé vìty a zkonstruuje z nich lingvistické stromy.

%\pagebreak

\item \textbf{Data extraction} \\ We use the structure of tectogrammatic (i.e. deep syntactic) dependency trees to extract relevant data. 
See section \ref{sec:extraction} for more details.

%Pomocí lingvistické struktury jednotlivých vìt extrahujeme data, která reprezentují informace vyjádøené v textu. Podrobnosti -- viz sekce \ref{sec:extraction}.

\item \textbf{Semantic representation} \\ This phase consists of quite simple data transformation or conversion to the desired ontology format. But it is quite important to choose suitable ontology that will properly represent semantics of the data. We have not implemented this phase yet.

%V této fázi sémanticky interpretujeme extrahovaná data pomocí konceptù vhodné ontologie.
\end{enumerate}
