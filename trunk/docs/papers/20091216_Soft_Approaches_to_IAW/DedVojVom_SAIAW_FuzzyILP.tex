...
\subsection{Translation of fuzzy ILP task to several crisp ILP tasks}
...
These two translation create two ILP tasks, the first one (\emph{raw}) is purely crisp and the second one (\emph{mon}) can be understood as (and translated back to) fILP.
...
\begin{itemize}
	...
	\item The \textit{mon ILP task} is given by ${B}^{mon}_T$ and $E_{\ge t}$ for each $t\in T$. As a~result it produces a set of hypotheses $H_{\ge t}$ guaranteeing examples of degree of at least $t$.
\end{itemize}

Note that variable boundings in $B$ have no boundings on the truth value attribute, which was added to each predicate, and hence there are no variable boundings on the truth value attribute in $H_{\ge t}$. We did not add the additional truth value attribute to the predicates in $E$ 

Now we sketch the translation of the \emph{mon ILP task} to GAP (fILP) rules. Let us assume that $C$ is the target predicate in the domain of ${\mathcal E}$. We define ${\mathcal H}$ with a domain consisting of one GAP rule $$C(y):u(x_1,\dots,x_m)\leftarrow B_1:x_1 \&\dots\& B_n:x_m,$$
here $B_1:x_1 \&\dots\& B_n:x_m$ is the enumeration of all predicates in $B$.
...
We claim, that if all $H_t$ were correctly learned by an crisp ILP system then for the minimal solution $u$ of all constraints $U_R$ the rule
$$
C(y):u(x_1,\dots,x_m)\leftarrow B_1:x_1 \&\dots\& B_n:x_m
$$
is a correct solution of the fILP task given by ${\mathcal E}$ and ${\mathcal B}$, for all $R\in H_t$ and $t\in T$. Our presentation is here a little bit simplified and we freely switch between fuzzy and GAP programs, which are known to be equivalent, see in~\citep{biblio:KLV}.
