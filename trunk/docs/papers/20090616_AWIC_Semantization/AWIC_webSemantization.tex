...
\authorrunning{Peter Vojt\'{a}\v{s} et al.}
% Use \authorrunning{Short Title} for an abbreviated version of
% your contribution title if the original one is too long
\institute{Jan Dedek \and Alan Eckhardt \and Peter Vojtas  \at Department of software engineering\\
Charles University,
...
\abstract{ Web Semantization is a concept we introduce in this paper. We understand Web Semantization as an automated process of increasing degree of semantic content on the web. Part the of content of the web is further usable, semantic content (usually annotated) is more suitable for machine processing.
...
\keywords {Semantic Web, Semantic Annotation, Web Information Extraction, User Preferences.}
...
\begin{figure}[bh]
\centering
\includegraphics[height=.8\hsize, width=.9\hsize]{img/Semantization}
...
Type of annotation&\textbf{Tabular pages}&\textbf{Textual pages}\\ \hline\hline
\textbf{Intermediate general}&Uses similarities&Does not use similarities\\ \hline
\textbf{Domain dependent}&Does not use similarities&Uses similarities\\ \hline
\end{tabular}	
%}
...
\begin{figure}
\centering
\includegraphics[width=.85\hsize]{img/UserSearch}
...
\begin{figure}[b]
\centering
\includegraphics[width=.7\hsize]{img/StructuralSimilarity}
...
\begin{figure}
\centering
\includegraphics[width=.65\hsize]{img/TectogramaticalTree}
...
\begin{figure}
\centering
\includegraphics[width=.33\hsize]{img/CarPrefs}
...
To conclude this section we refer to two survey papers about Web Information Extraction Systems \cite{biblio:Chang2006} and Semantic Annotation Platforms \cite{biblio:SemAnotSurvey}, which describe many other systems that are compatible with the idea of Web Semantization and can be also exploited in the process of gradual semantization of the web. As we mentioned above the domain dependent IE systems need initial learning for each new task. We take it into account and propose to use the learned systems only for suitable web pages selected by a prompt classifier.
...
\bibliography{AWIC_webSemantization}
