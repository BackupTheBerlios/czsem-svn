...
\begin{figure}
\centerline{\includegraphics[width=\hsize,height=\hsize]{img/rules}}
...
(1) experiments with $E_t$ and $B^{raw}_{T}$ and (2) experiments with $E_{\ge t}, {B}^{mon}_T$.
The Fig~\ref{img:evaluation} shows the success these rules on the test data (not used during learning). Naturally monotonized hypotheses $H_{\ge t}$ were tested on monotonized examples $E_{\ge t}$ (double-line frame in the Fig~\ref{img:rules}) and similarly for the for the crisp case $H_t, E_t$ (dashed-line frame).
