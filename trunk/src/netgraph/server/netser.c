			...
			if (childpid==0) { // potomek - ten bude obhospodařovat toto jedno spojení s klientem
				...
				if ((childpid = fork()) < 0) {
					...
					if (childpid > 0) { /* rodic */
						...
						close (mypipe2[1]); // zavřu výstup do roury 2, ze které budu číst
						close(0); // zavřu stdin
						dup(mypipe2[0]); // a připojím ho na rouru mypipe2
						close(mypipe2[0]); // a opět pro pořádek zavřu ten původní file descriptor
...
*/
						...
						//close (writefd);
						//close (readfd);
						close (mypipe2[0]); // zavřu čtení z roury, do které budu zapisovat
						close(1); // zavřu stdout
						dup(mypipe2[1]); // a připojím na něj rouru mypipe2
						close(mypipe2[1]); // ten původní file descriptor zavřu (pro pořádek)
